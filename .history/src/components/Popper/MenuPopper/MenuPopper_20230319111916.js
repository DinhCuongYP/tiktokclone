...
function MenuPopper({ children, items = [], handleClickMenu, customTippy = {} }) {
    ...
    const handleBackMenu = () => {
        ...
        newListMenu.pop();
        return setMenuList(newListMenu);
    };
    ...
    const renderMenu = (attrs) => (
        <nav className={cx('wrapper-menu')} tabIndex="-1" {...attrs}>
            <div data-popper-arrow className={cx('arrow-poper')} />
            <WrapperPopper className={cx('menu-list', { 'sub-menuChildren': menuList.length > 1 })}>
                {menuList.length > 1 && <MenuHeader title={menuTitle} onBack={handleBackMenu} />}
