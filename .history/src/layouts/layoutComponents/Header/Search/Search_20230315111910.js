...
function Search() {
    ...
    const handleChangeSearchValue = (e) => {
        ...
        valueSearchInput.starWidth(' ') || setSearchValue(valueSearchInput);
    };
        ...
        <div className={cx('search-result')} tabIndex="-1" {...attrs}>
            <WrapperPopper>
                ...
                {searchResult.map((accounItem) => (
                    <SearchAccountItem key={accounItem.id} accountInfo={accounItem} />
                ))}
            </WrapperPopper>
        </div>
    );
    ...
    return (
        <Tippy interactive visible={showSearch && !!searchValue && searchResult.length > 0} render={renderSearchResult}>
            <form ref={formRef} className={cx('wrapper')}>
                <input
                    ref={inputRef}
